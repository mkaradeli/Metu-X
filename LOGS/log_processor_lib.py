...
class  log_processor():
	...
	def convertBinary(self)-> None:
		if self.formatID == 0:
		...
		elif self.formatID == 14:   # packed, start[2] sync, ActuatorData_t[4], crc
			struct_size = self.file.read(2)
			self.struct_size, = struct.unpack('<H', struct_size)
			self.file.seek(512)          # skip the padded header
			...
			for i in range(N_ACT):
				...
				df_i.columns = ["timestamp"] + ACT_FIELDS
				self.df_act.append(df_i)
		elif self.formatID == 15:   # packed, start[2] sync, ActuatorData_t[4], crc
			struct_size = self.file.read(2)
			self.struct_size, = struct.unpack('<H', struct_size)
			self.file.seek(512)          # skip the padded header
			...
			# 2x skips start[2] sync bytes; tail: manifold_pressure, manifold_raw,
			# thrust_measured, thrust_raw, crc
			self.struct_format = '<2xI' + ACT_FMT * N_ACT + 'fHfHfffffffH'   # 280 bytes
			if self.struct_size != struct.calcsize(self.struct_format):
				print('struct format is wrong.')
				print(f'log struct size = {self.struct_size}, calculated struct size = {struct.calcsize(self.struct_format)}')
				exit()
			...
			for i in range(N_ACT):
				...
				df_i.columns = ["timestamp"] + ACT_FIELDS
				self.df_act.append(df_i)
		elif self.formatID == 16:   # packed, start[2] sync, ActuatorData_t[4], crc
			struct_size = self.file.read(2)
			self.struct_size, = struct.unpack('<H', struct_size)
			self.file.seek(512)          # skip the padded header
			...
			for i in range(N_ACT):
				...
				df_i.columns = ["timestamp"] + ACT_FIELDS
				self.df_act.append(df_i)
		elif self.formatID == 17:   # packed, start[2] sync, ActuatorData_t[4], crc
			struct_size = self.file.read(2)
			self.struct_size, = struct.unpack('<H', struct_size)
			self.file.seek(512)          # skip the padded header
			...
			self.binary = self.file.read()
			N_ACT = 4
			# ActuatorData_t (packed): 14 floats = 56 bytes
			ACT_FMT = '14f'
			ACT_FIELDS = [
				"current_measured",
				"current_demand",
			...
			N_ACT_FIELDS = len(ACT_FIELDS)   # 14 unpacked values per actuator

			# 2x skips start[2] sync bytes; tail: manifold_pressure, thrust_measured,
			# quaternion, linAccel, lidar, kf_altitude/velocity, 6 mode/status bytes, crc
			self.struct_format = '<2xI' + ACT_FMT * N_ACT + '2f4f3f2H2f6BfH'   # 286 bytes
			if self.struct_size != struct.calcsize(self.struct_format):
				print('struct format is wrong.')
				print(f'log struct size = {self.struct_size}, calculated struct size = {struct.calcsize(self.struct_format)}')
				exit()
			...
			for i in range(N_ACT):
				...
				df_i.columns = ["timestamp"] + ACT_FIELDS
				self.df_act.append(df_i)
		elif self.formatID == 18:   # packed, start[2] sync, ActuatorData_t[4], crc
			struct_size = self.file.read(2)
			self.struct_size, = struct.unpack('<H', struct_size)
			self.file.seek(512)          # skip the padded header
			...
			# 2x skips start[2] sync bytes; tail: manifold_pressure, thrust_measured,
			# quaternion, linAccel, lidar, kf_altitude/velocity, 6 mode/status bytes, crc
			self.struct_format = '<2xI' + ACT_FMT * N_ACT + '2f4f3f6f2H2f6BfH'   # 286 bytes
			if self.struct_size != struct.calcsize(self.struct_format):
				print('struct format is wrong.')
				print(f'log struct size = {self.struct_size}, calculated struct size = {struct.calcsize(self.struct_format)}')
				exit()
			...
			self.columns += [
				...
				"angular_velocity_y",
				"angular_velocity_z",
				"angular_velocity_demand_x",
				"angular_velocity_demand_y",
				"angular_velocity_demand_z",
			...
			for i in range(N_ACT):
				...
				df_i.columns = ["timestamp"] + ACT_FIELDS
				self.df_act.append(df_i)
		elif self.formatID == 19:   # packed, start[2] sync, ActuatorData_t[4], crc
			struct_size = self.file.read(2)
			self.struct_size, = struct.unpack('<H', struct_size)
			self.file.seek(512)          # skip the padded header
			...
			# 2x skips start[2] sync bytes; tail: manifold_pressure, thrust_measured,
			# quaternion, linAccel, lidar, kf_altitude/velocity, 6 mode/status bytes, crc
			self.struct_format = '<2xI' + ACT_FMT * N_ACT + '2f4f3f6f2H2f6B3ffH'   # 286 bytes
			if self.struct_size != struct.calcsize(self.struct_format):
				print('struct format is wrong.')
				print(f'log struct size = {self.struct_size}, calculated struct size = {struct.calcsize(self.struct_format)}')
				exit()
			...
			for i in range(N_ACT):
				...
				df_i.columns = ["timestamp"] + ACT_FIELDS
				self.df_act.append(df_i)
		elif self.formatID == 20:   # packed, start[2] sync, ActuatorData_t[4], crc
			struct_size = self.file.read(2)
			self.struct_size, = struct.unpack('<H', struct_size)
			self.file.seek(512)          # skip the padded header
			...
			# 2x skips start[2] sync bytes; tail: manifold_pressure, thrust_measured,
			# quaternion, linAccel, lidar, kf_altitude/velocity, 6 mode/status bytes, crc
			self.struct_format = '<2xI' + ACT_FMT * N_ACT + '2f4f3f6f2H2f6B3f4ffH'   # 286 bytes
			if self.struct_size != struct.calcsize(self.struct_format):
				print('struct format is wrong.')
				print(f'log struct size = {self.struct_size}, calculated struct size = {struct.calcsize(self.struct_format)}')
				exit()
			...
			for i in range(N_ACT):
				...
				df_i.columns = ["timestamp"] + ACT_FIELDS
				self.df_act.append(df_i)
		elif self.formatID == 21:   # packed, start[2] sync, ActuatorData_t[4], crc
			struct_size = self.file.read(2)
			self.struct_size, = struct.unpack('<H', struct_size)
			self.file.seek(512)          # skip the padded header
			...
			# 2x skips start[2] sync bytes; tail: manifold_pressure, thrust_measured,
			# quaternion, linAccel, lidar, kf_altitude/velocity, 6 mode/status bytes, crc
			self.struct_format = '<2xI' + ACT_FMT * N_ACT + '2f4f3f6f2H2f6B3f4f2ffH'   # 286 bytes
			if self.struct_size != struct.calcsize(self.struct_format):
				print('struct format is wrong.')
				print(f'log struct size = {self.struct_size}, calculated struct size = {struct.calcsize(self.struct_format)}')
				exit()
