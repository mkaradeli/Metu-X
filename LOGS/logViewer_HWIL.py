...
if len(sys.argv) == 2:
	filename = sys.argv[1]
print(filename)
filename = "tlm_20260903_122810.bin"
...
ax4.grid(True)
ax4.set_title('kf_altitude')
ax4.plot(myLog.df.timestamp, myLog.df.kf_altitude, label="kf_altitude")
ax4.set_ylabel('kf_altitude (m) bro')
...
ax5.grid(True)
ax5.set_title('kf_velocity')
ax5.plot(myLog.df.timestamp, myLog.df.kf_velocity, label="kf_velocity")
ax5.set_ylabel('kf_velocity (m/s) bro')
...
ax8.plot(myLog.df.timestamp, myLog.df.linAccel_x * 38.3, label="Accel X")
ax8.plot(myLog.df.timestamp, myLog.df.linAccel_y * 38.3, label="Accel Y")
ax8.plot(myLog.df.timestamp, myLog.df.linAccel_z * 38.3 - 9.81 * 38.3, label="Accel Z")

ax8.set_xlabel("Time (s)")
ax8.set_ylabel('HWIL Force Estimated')
